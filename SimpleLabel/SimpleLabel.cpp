...
void SimpleLabel::on_actionOpen_triggered()
{
	int frames = 0;
	int firstframe = 0;
	QString s = QFileDialog::getOpenFileName(this, tr("Open Video"), ".", tr("Image Files (*.png *.tif *.tiff);;Video Files (*.avi)"));
	if(!s.isEmpty())
	{
		resetFilenames();
		resetLabels();
	...
	//		m_Monitor->setLoadBackground(ui.chkBox_Adapt2Bkgd->isChecked());
		...
		else if(mExtention == ".tif" || mExtention == ".tiff" || mExtention == ".png")
		{
